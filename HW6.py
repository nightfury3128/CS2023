...
def has_prefix(s, prefix):
    ...
    if prefix == empty:
        return True
    elif s == empty or first(s) != first(prefix):
        return False
    else:
...
def count_change(amount, denominations):
    ...
    if amount == 0:
        ...
        with_first = count_change(amount - first(denominations), denominations)
        without_first = count_change(amount, rest(denominations))
